...
6. **Cancellation with terminal-state guard.** `cancel(task_id)` returns the spec JSON-RPC `-32602` code when invoked on a terminal task. Terminal tasks never transition further — enforced both in the entity and at the SQL layer.
